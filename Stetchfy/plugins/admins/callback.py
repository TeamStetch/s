...
from Stetchfy import YouTube, app
from Stetchfy.core.call import Stetch
from Stetchfy.misc import SUDOERS, db
...
from Stetchfy.utils.decorators.language import languageCB
from Stetchfy.utils.formatters import seconds_to_min
from Stetchfy.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from Stetchfy.utils.stream.autoclear import auto_clean
from Stetchfy.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Stetch.mute_stream(chat_id)
        ...
        await Stetch.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Stetch.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Stetch.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Stetch.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Stetch.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Stetch.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
